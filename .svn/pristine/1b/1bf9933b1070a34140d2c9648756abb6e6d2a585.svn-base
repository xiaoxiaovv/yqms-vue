	...
	<div>
		...
		<div class="dialog_footer flex">
			...
			<div class="dialog_footer_r item_vertical">
				<div class="flex">
					<div class="btn reset_btn" v-if="isRestore && !($store.state.kuTempLateType == 0 && $store.state.agentid > 0)" @click="restoreDefaultHeader">恢复默认</div>
	...
	export default {
		...
		created() {
			...
			if((_this.$store.state.kuTempLateType == 0 || _this.$store.state.kuTempLateType == 1) && _this.$store.state.agentid > 0){
				_this.headerDataTemp.pic = _this.headerDataTemp.pic.filter((item) => {
