	...
	<div>
		<div class="dialog_center">
			...
			<div class="flex top_info">
				...
				<div class="flex">
                    <!-- <div class="flex draggable_pic">
						<div class="pic_item cursor_default" v-for="item of headerDataTemp.pic" :key="item.id" :title="picTitle[item.id]">
					...
					<draggable class="flex draggable_pic" v-model="headerDataTemp.pic" :options="{group:'pic'}" @start="drag=true" @end="drag=false">
						<div class="pic_item flex flex_center item_center" v-for="item of headerDataTemp.pic" :key="item.id" :title="picTitle[item.id]" @mouseleave.prevent="handleMouseLeavePic($event)" @mouseenter.prevent="handleMouseEnterPic($event)" v-show='picTitle[item.id] == "联系客服"'>
							<div class="icon_cover icon_cover_data hide"></div>
							<div class="icon_eye hide"></div>
							<div class="icon_bg" :class="'icon'+item.id"></div>
							<div>{{picTitle[item.id]}}</div>
						</div>
					</draggable>
					<div class="user_box">
		...
		<div class="dialog_footer flex">
			...
			<div class="dialog_footer_r item_vertical">
				<div class="flex">
					<!-- 慕亚不能恢复默认 -->
					<!-- <div class="btn reset_btn" v-if="isRestore" @click="restoreDefaultHeader">恢复默认</div>
					<div v-else class="btn reset_btn">恢复默认</div> -->
...
<script>
	...
	export default {
		...
		created() {
			...
			_this.headerDataTemp = Object.assign(_this.headerData);
		...
		methods: {
			...
			handleBlur(event, item, index) {
				let curEle = event.currentTarget;
				curEle.parentElement.classList.remove('color_white');
			...
			hanldleClickEdit(event) {
				...
				curEle.previousElementSibling.parentElement.classList.add('color_white');
				curEle.parentElement.parentElement.classList.add('nav_item_border');
			},
			end: function (evt) {
			      evt.item.children[2].classList.remove('color_white')
			},
			//恢复默认
			...
			handleOk() {
				console.log('121')
				let _this = this;
				_this.headerDataTemp.label.name == '' ? '舆情秘书' : _this.headerDataTemp.label.name;
				_this.handleNavSort();
				_this.handlePicSort();
				...
				let noNavData = localStorage.getItem('noNavData');
				let navnew = [];
				let i = 0;
...
<style scoped>
	@import url("./definedHeaderIsentia.css");
</style>
